...
if switch_plot_aero:
    ...
    for i in range(n_points):
        ...
        F_lst_aero[2, i] += T_damaged.flatten()
        M_lst_aero[2, i] += Q_damaged.flatten()
        rotation_angle_lst[i] = rotation_angle
        rotation_angle = propeller.update_rotation_angle(omega_local, dt)
    ...
    F_aero_V0_0 = np.load("Plot_data_storage/F_aero_v0_0.npy")
    M_aero_V0_0 = np.load("Plot_data_storage/M_aero_v0_0.npy")
...
M_complete = np.load("Plot_data_storage/complete.npy")
M_S = np.load("Plot_data_storage/no_S.npy")
M_vi = np.load("Plot_data_storage/no_vi.npy")
M_S_vi = np.load("Plot_data_storage/no_S_no_vi.npy")

plt.figure(124)

plt.plot(percentage_broken_blade_length_lst[1:], (M_complete[0, 1:, 0] - M_complete[0, :-1, 0]) / 5, linewidth=4, color='#1f77b4', label="Upper limit")
plt.plot(percentage_broken_blade_length_lst[1:], (M_complete[0, 1:, 1] - M_complete[0, :-1, 1]) / 5, linewidth=4, color='#ff7f0e', label="Lower limit")

plt.plot(percentage_broken_blade_length_lst[1:], (M_S[0, 1:, 0] - M_S[0, :-1, 0]) / 5, linewidth=4, color='#2ca02c', label="S=const", linestyle='--', alpha=0.7)
plt.plot(percentage_broken_blade_length_lst[1:], (M_S[0, 1:, 1] - M_S[0, :-1, 1]) / 5, linewidth=4, color='#2ca02c', linestyle='--', alpha=0.7)

plt.plot(percentage_broken_blade_length_lst[1:], (M_vi[0, 1:, 0] - M_vi[0, :-1, 0]) / 5, linewidth=4, color='#d62728', label="$v_i=0$", linestyle='dotted', alpha=0.7)
plt.plot(percentage_broken_blade_length_lst[1:], (M_vi[0, 1:, 1] - M_vi[0, :-1, 1]) / 5, linewidth=4, color='#d62728', linestyle='dotted', alpha=0.7)

plt.plot(percentage_broken_blade_length_lst[1:], (M_S_vi[0, 1:, 0] - M_S_vi[0, :-1, 0]) / 5, linewidth=4, color='#9467bd', label="S=const & $v_i=0$", linestyle='-.', alpha=0.7)
plt.plot(percentage_broken_blade_length_lst[1:], (M_S_vi[0, 1:, 1] - M_S_vi[0, :-1, 1]) / 5, linewidth=4, color='#9467bd', linestyle='-.', alpha=0.7)


plt.xlabel("BD [%]")
plt.ylabel("$\\frac{d}{d\,BD}\Delta M^B_x$ [Nm/%]")
plt.ticklabel_format(axis="y", style="sci", scilimits=(-2, -7))
plt.ylim([-0.0021, 0.0040])
plt.grid(True)
plt.legend()
