...
if coefficients_identification:
    coeffs, A, b = propeller.compute_cla_coeffs(number_samples, n_blade_segment, degree_cla, degree_cda, min_w=min_w,
                                                max_w=max_w, va=va, rho=1.225, activate_plotting=True,
                                                activate_params_blade_contribution_plotting=activate_params_blade_contribution_plotting,
                                                LS_method="GLS", start_plot=start_cla_plot, finish_plot=finish_cla_plot)
    cla_coeffs = coeffs[:degree_cla+1, 0]
    cda_coeffs = coeffs[degree_cla+1:, 0]
    print(cla_coeffs)
    print(cda_coeffs)

# Compute the thrust generated from only taking into account the lift forces generated by the propellers
...
Q_remaining, Q_damaged, F_remaining, F_damaged = propeller.compute_torque_force(n_blade_segment, omega, propeller_speed, cla_coeffs, cda_coeffs)
